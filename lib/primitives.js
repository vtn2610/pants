...
(function (Primitives) {
            ...
            return (istream) => {
                ...
                switch (outcome.tag) {
                    ...
                    case "failure":
                        ...
                        let edits = editParse(parser, istream, outcome.error.edit, outcome.error.expectedStr().length, []);
    ...
    function char(c) {
        ...
        return expect(_sat([c]))((error) => new CharError_1.CharError(c, error.edit));
    }
    ...
    function editParse(p, istream, LCS, windowSize, edits) {
        if (!istream.isEmpty()) {
    ...
    function eof() {
        return (istream) => {
            return istream.isEOF() ? new Success(istream, Primitives.EOF) : new Failure(istream, istream.startpos, new StringError_1.StringError("EOF", istream.length()));
        };
    }
    Primitives.eof = eof;
    /**

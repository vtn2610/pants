...
(function (Primitives) {
    ...
    function editParse(p, LCS = 0, istream, errorpos, windowSize, edits) {
        if (!istream.isEmpty()) {
            let o = p(istream);
            switch (o.tag) {
                case "success":
                    ...
                    // if edits.length > 0 and error at current pos is not fixed, shift error list and fix it
                        ...
                        if (curEdit !== undefined && curEdit.sign === true) {
                            ...
                            for (let item of edits)
                                item.pos++; //increment all the remaining pos 
                            // case of deletion
                        }
                            ...
                            for (let item of edits)
                                item.pos--; //decrement all the remaining pos
                        }
                        LCS++;
    ...
    function eof() {
        return (istream) => {
            return istream.isEOF() ? new Success(istream, Primitives.EOF) : new Failure(istream, istream.startpos, new StringError_1.StringError(" "));
        };
    }
    Primitives.eof = eof;
    /**

...
(function (Primitives) {
            ...
            return (istream) => {
                ...
                switch (outcome.tag) {
                    case "success":
                        return outcome;
                    case "failure":
                        //let oEdit : [number,string] = LCSParse(parser, istream)
    ...
    function item() {
        return (istream) => {
            if (istream.isEmpty()) {
                return new Failure(istream, istream.startpos, new ItemError_1.ItemError(0));
            }
            else {
    ...
    function sat(char_class) {
        let f = (x) => {
            return (char_class.indexOf(x.toString()) > -1)
                ? result(x)
                : (istream) => new Failure(istream, istream.startpos - 1, new SatError_1.SatError(char_class, 0));
        };
        return bind(item())(f);
    }
    Primitives.sat = sat;
    ...
    function char(c) {
        ...
        return expect(sat([c]))((error) => new CharError_1.CharError(c, 0));
    }
    ...
    function letter() {
        ...
        return expect(parser)((error) => new LetterError_1.LetterError(0));
    }
    ...
    function digit() {
        ...
        return expect(parser)((error) => new DigitError_1.DigitError(0));
    }
    ...
    function editParse(p, LCS, istream) {
        ...
        if (!istream2.isEmpty()) {
            ...
            switch (o.tag) {
                ...
                case "failure":
                    ...
                    return editParse(p, LCS, istream2);
    ...
    function str(s) {
        return (istream) => {
            ...
            return expect(p)((error) => new StringError_1.StringError(s, 0))(istream);
        };
    }
    Primitives.str = str;
    /**
    ...
    function eof() {
        return (istream) => {
            return istream.isEOF() ? new Success(istream, Primitives.EOF) : new Failure(istream, istream.startpos, new StringError_1.StringError("EOF Error", 0));
        };
    }
    Primitives.eof = eof;
    /**
    ...
    function between(popen) {
        return (pclose) => {
            return (p) => {
                let l = left(p)(expect(pclose)((error) => new BetweenRightError_1.BetweenRightError(error, 0)));
                let r = right(expect(popen)((error) => new BetweenLeftError_1.BetweenLeftError(error, 0)))(l);
                return r;
            };
        };
    }
    ...
    function ws1() {
        return (istream) => {
            let o = expect(many1(wschars))((error) => new WSError_1.WSError(0))(istream);
            switch (o.tag) {
                case "success":
                    return new Success(o.inputstream, CharStream.concat(o.result));
                case "failure":
